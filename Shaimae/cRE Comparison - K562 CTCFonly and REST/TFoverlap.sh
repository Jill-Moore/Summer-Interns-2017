...
# Script to find the overlaps between TFs and regions of K562 CTCF-only cREs that overlap with REST peaks

# Prompt the user to enter the name of the file for TF overlapping analysis
if [ $# != 1 ]
then
        echo Enter the name of the file for which you want to perform the TF overlap.
	exit
fi

# Get ENCODE file information (experiment ID, peak file ID, TF name) from the text file
awk '{print $1}' ./K562-TF.txt > experimentNamesTF.txt
awk '{print $2}' ./K562-TF.txt > peakFileNamesTF.txt
awk '{print $3}' ./K562-TF.txt > targetNamesTF.txt

# Create arrays for each of the three types of file information from above
experiment=( `cat "experimentNamesTF.txt" `)
peakFile=( `cat "peakFileNamesTF.txt" `)
target=( `cat "targetNamesTF.txt" `)

# Strip the ".bed" extension from the cRE file name
fileName=`basename $1 .bed`

# Set up the header of the results file
echo -e Experiment '\t' Peak File '\t' Target '\t' \# Overlaps '\t' Percent Overlap >> ./results/TF$fileName.txt

# Find the number of TFs that will be intersected
numberOfTFs=`cat "targetNamesTF.txt" | wc -l`
...
do
	# Find the number of intersections between the CTCF-only cREs that overlap with REST peaks and the TFs
	numberOfOverlaps=$( bedtools intersect -u -a ./$1 -b ../K562-TF-Overlap/K562TFfiles/${peakFile[$i]}.bed | wc -l )
	# Find the total number of CTCF-only cREs that overlap with REST peaks
	totalcREs=$( wc -l ./$1 | cut -f 1 -d ' ')
	# Calculate the percent overlap between the cREs and the TFs
	percentOverlap=$(awk "BEGIN {printf \"%.2f\", $numberOfOverlaps / $totalcREs * 100 }")

	echo -e ${experiment[$i]} '\t' ${peakFile[$i]} '\t' ${target[$i]} '\t' $numberOfOverlaps '\t' $percentOverlap >> ./results/TF$fileName.txt
