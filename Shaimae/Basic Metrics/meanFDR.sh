...
# Create an array of all GM12878 and K562 Histone Mark file names
...
echo -e Experiment '\t' Peak File '\t' Target '\t' Average Peak Width '\t' >> meanFDR_Results.txt

# Cycle through all of the files to find the average FDR
...
do
	...
	if [ $i -le 11 ]
	then
		# Isolate 9th column containing FDR values
		awk '{print $9}' ~/finalScripts/GM12878-Histone-Overlap/GM12878HistoneFiles/${peakFile[$i]} > FDR$i.txt
	# for K562 files (listed after GM12878 in the text file)
	else 
		# Isolate 9th column containing FDR values
		awk '{print $9}' ~/finalScripts/K562-Histone-Overlap/K562HistoneFiles/${peakFile[$i]} > FDR$i.txt
	fi

	# Find the average of the FDRs and assign to a variable
	meanFDR=`awk '{ sum += $1; n++ } END { if (n > 0) print sum / n; }' FDR$i.txt`
	# Send the values to the results file
	echo -e ${experiment[$i]} '\t' ${peakFile[$i]} '\t' ${target[$i]} '\t' $meanFDR >> ./meanFDR_Results.txt
	# Clean up
	rm FDR$i.txt
done
