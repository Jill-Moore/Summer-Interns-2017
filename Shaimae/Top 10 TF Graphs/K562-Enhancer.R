...
# Read in the data
K562_E <- read.csv("K562-Enhancer.csv", header = TRUE)

# Isolate the TFs
all_TFs <- K562_E$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- K562_E$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- K562_E$X.5[-c(1,1)]
...
# Create a data frame
K562_E <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)
...
g <- ggplot(K562_E, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
png("K562-Enhancer.png")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = K562_E$Ratios), 
             color = "black",
             size = 0.3) + 
