...
# Read in the data
K562_D2 <- read.csv("K562-D2.csv", header = TRUE)

# Isolate the TFs
all_TFs <- K562_D2$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- K562_D2$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- K562_D2$X.5[-c(1,1)]
...
# Create a data frame
K562_D2 <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)
...
g <- ggplot(K562_D2, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
png("K562-DNase-Group2.png")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = K562_D2$Ratios),
             color = "black",
             size = 0.3) + 
  labs(title = "K562 DNase Group 2",
       subtitle = "Top 10 TFs",
