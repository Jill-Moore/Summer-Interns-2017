...
# Read in the data
GM12878_CTCF <- read.csv("GM12878-CTCF.csv", header = TRUE)

# Isolate the TFs
all_TFs <- GM12878_CTCF$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- GM12878_CTCF$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- GM12878_CTCF$X.5[-c(1,1)]
...
# Create a data frame
GM12878_CTCF <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)

# View the data frame
GM12878_CTCF

# Set the palette (appropriate colors)
P_palette <- c("gray60", "white", "#00B0F0")

# Set the theme
theme_set(theme_classic())

# Initialize the plot (set x and y variables)
g <- ggplot(GM12878_CTCF, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
pdf("GM12878-CTCF.pdf")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = GM12878_CTCF$Ratios),
             color = "black",
             size = 0.3) + 
  labs(title = "GM12878 CTCF",
       subtitle = "Top 10 TFs",
       caption = "*Ratio = percent overlap (CTCF) : percent overlap (aggreagate of other categories)",
  ...
  scale_fill_gradientn(colors = P_palette, guide = "colorbar",
                       limits = c(-3.5, 3.5), name = "log2(Ratio*)") +
