...
# Read in the data
K562_CTCF <- read.csv("K562-CTCF.csv", header = TRUE)

# Isolate the TFs
all_TFs <- K562_CTCF$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- K562_CTCF$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- K562_CTCF$X.5[-c(1,1)]
...
# Create a data frame
K562_CTCF <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)
...
g <- ggplot(K562_CTCF, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
png("K562-CTCF.png")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = K562_CTCF$Ratios),
             color = "black",
             size = 0.3) + 
  labs(title = "K562 CTCF",
       subtitle = "Top 10 TFs",
