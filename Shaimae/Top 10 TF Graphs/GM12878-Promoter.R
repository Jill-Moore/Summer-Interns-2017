...
# Read in the data
GM12878_P <- read.csv("GM12878-Promoter.csv", header = TRUE)

# Isolate the TFs
all_TFs <- GM12878_P$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- GM12878_P$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- GM12878_P$X.5[-c(1,1)]
...
# Create a data frame
GM12878_P <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)
...
g <- ggplot(GM12878_P, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
png("GM12878-Promoter.png")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = GM12878_P$Ratios),
             color = "black",
             size = 0.3) + 
