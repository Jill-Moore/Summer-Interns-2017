...
# Read in the data
GM12878_E <- read.csv("GM12878-Enhancer.csv", header = TRUE)

# Isolate the TFs
all_TFs <- GM12878_E$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- GM12878_E$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- GM12878_E$X.5[-c(1,1)]
...
# Create a data frame
GM12878_E <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)

# View the data frame
GM12878_E

# Set the palette (appropriate colors)
P_palette <- c("gray60", "white", "#FFCD00")

# Set the theme
theme_set(theme_classic())

# Initialize the plot (set x and y variables)
g <- ggplot(GM12878_E, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
pdf("GM12878-Enhancer.pdf")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = GM12878_E$Ratios),
             color = "black",
             size = 0.3) + 
  labs(title = "GM12878 Enhancer-like",
       subtitle = "Top 10 TFs",
       caption = "*Ratio = percent overlap (Enhancer-like) : percent overlap (aggreagate of other categories)",
