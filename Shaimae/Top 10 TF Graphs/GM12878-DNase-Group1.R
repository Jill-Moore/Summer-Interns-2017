...
# Read in the data
GM12878_D1 <- read.csv("GM12878-D1.csv", header = TRUE)

# Isolate the TFs
all_TFs <- GM12878_D1$X.1[-c(1,1)]
# Isolate the fraction of overlaps
all_Overlaps <- GM12878_D1$X.3[-c(1,1)]
# Isolate the log2(ratios)
all_Ratios <- GM12878_D1$X.5[-c(1,1)]
...
# Create a data frame
GM12878_D1 <- data.frame(TFs, Fraction_Overlap, Ratios, stringsAsFactors = FALSE)
...
g <- ggplot(GM12878_D1, aes(x = TFs, y = Fraction_Overlap))

# Initialize the file
pdf("GM12878-DNase-Group1.pdf")

# Plot
g + geom_bar(stat = "identity",  width = 0.75, aes(fill = GM12878_D1$Ratios),
             color = "black",
             size = 0.3) + 
  labs(title = "GM12878 DNase Group 1",
       subtitle = "Top 10 TFs",
       caption = "*Ratio = percent overlap (DNase Group 1) : percent overlap (aggreagate of other categories)",
