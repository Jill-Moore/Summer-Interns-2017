...
with open('./chiSquare-WilcoxData/histoneoverlap_Promoter-POL2_GM12878.txt') as f:
        content = f.readlines()
content=[x.strip() for x in content]
promoterOverlap = [int(value) for value in content]

with open('./chiSquare-WilcoxData/histonenoOverlap_Promoter-POL2_GM12878.txt') as f:
        content = f.readlines()
content=[x.strip() for x in content]
promoterNoOverlap = [int(value) for value in content]

with open('./chiSquare-WilcoxData/histoneoverlap_Enhancer-POL2_GM12878.txt') as f:
	content = f.readlines()
content=[x.strip() for x in content]
enhancerOverlap = [int(value) for value in content]

with open('./chiSquare-WilcoxData/histonenoOverlap_Enhancer-POL2_GM12878.txt') as f:
	content = f.readlines()
content=[x.strip() for x in content]
...
for i in range(0, count):
	...
	if promoterOverlap[i] == 0 | promoterNoOverlap[i] == 0:
		...
		# Write the results to a file
                f= open("./GM12878results/promoterChiSquare.txt", "a+")
	...
	if enhancerOverlap[i] == 0 | enhancerOverlap[i] == 0:
		...
		# Perform the chi-square test using built-in function (contingency table method)
                chi, p, dof, ex = scipy.stats.chi2_contingency(observed) # correction=True
                # Write the results to a file
                f= open("./GM12878results/enhancerChiSquare.txt", "a+")
                f.write(experiment[i])
                f.write("\t")
                f.write(peakFile[i])
                f.write("\t")
